...
class ActionLocationSolt(Action):

    def name(self) -> Text:
        return "action_location_slot"
    
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:

        data = tracker.get_slot("location_slot")
        print(data)
        dispatcher.utter_message(text=data)
...
def get_url(code, id):
    ...
    url = "http://www.kuaidi.com/index-ajaxselectcourierinfo-" + id + "-" + code + "-UUCAO" + time_new + ".html"
    return url
